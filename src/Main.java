import java.util.Arrays;

public class Main {
    public static void main(String[] args) {
//        Threesum threesum = new Threesum();
//        threesum.threeSum(new int[]{1, 2, 3, 4, 5, 6, 7, 8, 9});
//        threesum.threeSum(new int[]{-1, 0, 1, 2, -1, -4});
//        threesum.threeSum(new int[]{0, 1, 1});
//        threesum.threeSum(new int[]{0, 0, 0});

//        MergeStrings mergeStrings = new MergeStrings();
//        System.out.println(mergeStrings.mergeAlternatively("abc", "pqr"));
//        System.out.println(mergeStrings.mergeAlternatively("ab", "pqrs"));
//        System.out.println(mergeStrings.mergeAlternatively("abcd", "pq"));

//        NumOfStrings numOfStrings = new NumOfStrings();
//        numOfStrings.numOfStrings(new String[]{"a", "abc", "bc", "d"}, "abc");
//        numOfStrings.numOfStrings(new String[]{"a", "b", "c"}, "aaaaaabbbbb");
//        numOfStrings.numOfStrings(new String[]{"a", "a", "a"}, "ab");

        int[] data = {-5, -10, -2, -4, 5, 200, 10, 100, -100};
        MergeSort.mergeSort(data, 0, data.length - 1);
        System.out.println(Arrays.toString(data));

//        System.out.println(Recursion.stringReversal("pussy"));
//        System.out.println(Recursion.stringReversal("racecar"));

        LongestConsecutive lg = new LongestConsecutive();
        System.out.println(lg.longestConsecutive(new int[]{100, 4, 200, 1, 3, 2}));
        System.out.println(lg.longestConsecutive(new int[]{100, 4, 200, 1, 3, 2}));
    }
}